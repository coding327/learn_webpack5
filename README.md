...
对于我们的项目，主要被分为开发环境和生产环境，运行我们目前的项目的时候有一个弊端，当我们进行打包时，它会加载`webpack.config.js`文件，但是这个文件里面也有很多时开发环境的配置，如`mode`为`development`，`devtool`，那么相当于他也会跑一遍，当我们运行本地服务时也有一些配置不合适，如清除打包的插件，复制功能插件
