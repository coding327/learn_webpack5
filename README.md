...
主要是因为`webpack`最新版本不需要设置`host: '0.0.0.0'`，默认就用`localhost`，这个`0.0.0.0`是在同一个网段下，另外一台电脑也能通过`ip`来访问我们应用【我使用时测试了一下，不设置`host: '0.0.0.0'`也能在另外一个台电脑上通过`ip`来访问，这个功能相当于在`webpack5`内置了】
